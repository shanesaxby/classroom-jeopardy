(() => {
  ...
  const BASE_RULES = {
    general: [
      "The team in control chooses a category and value.",
  ...
  function resolveFirstAttemptCorrect() {
    ...
    const team = state.teams[state.controlTeamIndex];
    team.score += value;
    ...
    showQuestionOutcome(`${team.name} is correct: +${formatPoints(value)} and keeps control.`, "success");
    renderScoreboard(state.controlTeamIndex, true);
    playSound("correct");
    saveGameState();
  }
  ...
  function renderReboundPanel() {
    ...
    state.teams.forEach((team, index) => {
      ...
      [
        ...
        button.addEventListener("click", () => setReboundResponse(index, value));
    ...
    el.selectReboundWinnerButton.textContent = count > 0
      ? `Randomly select from ${count} correct team${count === 1 ? "" : "s"}`
      : "Randomly select rebound";
    ...
    el.noCorrectReboundButton.textContent = incorrectTeams.length
      ...
      : "No correct rebound";
    el.reboundRuleSummary.textContent = reboundModeLabel(state.gameRules.reboundMode);
  }
  ...
  function resolveRebound() {
    ...
    state.controlTeamIndex = winnerIndex;
    state.questionResolved = true;
    state.questionPhase = "resolved";
    ...
    showQuestionOutcome(`${winner.name} wins the rebound and takes control.${scoreText}${deductionText}`, "success");
    playSound("rebound");
    saveGameState();
  }
  ...
  function resolveNoRebound() {
    ...
    renderScoreboard();
    const deductionText = deductionNotes.length ? ` Deductions: ${deductionNotes.join(" · ")}.` : "";
    showQuestionOutcome(`No correct rebound. ${controlTeam.name} keeps control.${deductionText}`, "neutral");
    playSound("close");
    saveGameState();
  }
  ...
  function currentDefaultRules() {
    const deductionRule = state.gameRules.incorrectFirstDeduction
      ? "If the team in control is incorrect, the clue value is deducted before the rebound. During the rebound, only teams explicitly marked Incorrect lose the clue value; No Answer is not penalised."
      : "If the team in control is incorrect, no points are deducted; the question moves to a rebound.";

    const reboundRule = {
      single: "On a rebound, the host marks each board Correct, Incorrect or No Answer. One correct team is chosen at random; that team gets the full clue value and takes control.",
      "all-full": "On a rebound, every correct team gets the full clue value. One correct team is chosen at random to take control.",
      shared: "On a rebound, correct teams share the clue value. One correct team is chosen at random to take control and receives any indivisible remainder.",
      fixed: `On a rebound, every correct team receives a fixed ${formatPoints(state.gameRules.fixedBonus)}-point bonus. One correct team is chosen at random to take control.`
    }[state.gameRules.reboundMode] || "On a rebound, one correct team is chosen at random for the clue value and control.";

    return {
      general: [...BASE_RULES.general],
      scoring: [
        "A correct first response earns the full clue value and the team keeps control.",
        deductionRule,
        reboundRule,
        "If there is no correct rebound, no rebound points are awarded and the original team keeps control. In deduction mode, any rebound team explicitly marked Incorrect still loses the clue value.",
  ...
  function restoreSavedGame() {
    ...
    if (!raw) {
      showToast("No Version 3/3.1 saved game was found.");
      return;
    }
